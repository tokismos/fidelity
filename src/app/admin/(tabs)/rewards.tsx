...
export default function Rewards() {
  ...
  const handleEdit = ({ rewardId, type }: { rewardId: string; type: string }) => {
  ...
  const renderRewardDetails = (reward: Reward) => {
    switch (reward.type) {
      case "BUY_N_GET_1":
        return (
          <Text className="text-sm text-gray-600">
            Buy {(reward.config as BuyNGet1Config).required_purchases} and get 1 free
          </Text>
        )
      ...
      case "DISCOUNT_FIX":
        return (
          <Text className="text-sm text-gray-600">${(reward.config as DiscountFixConfig).discount_amount} off</Text>
        )
      case "FREE_ITEM":
        return <Text className="text-sm text-gray-600">Free {(reward.config as FreeItemConfig).item_name}</Text>
  ...
  const renderIcon = (reward: Reward) => {
    if (reward.config.image_path) {
      return <Image source={{ uri: reward.config.image_path }} className="h-8 w-8 rounded-full" />
    }
