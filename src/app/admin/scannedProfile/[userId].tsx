...
export default function UserProfile() {
  ...
  const { data: redeemedRewards, error: redeemedRewardsError } =
    useGetRedeemedRewards({ storeId, userId });
  const { userPoints, isLoading, error, isFetching, data } = useGetPoints({
    userId,
  });
  const {
    addUserToStore,
    isPending,
    error: userToStoreError,
  } = useAddUserToStore();
  const { updatePoints, error: updatePointsError } = useUpdatePoints({
    userId,
    storeId,
  });
  ...
  const showConfirmationDialog = useCallback(
    (points: number) => {
      Alert.alert(
        'Confirm Transaction',
        `${isAddMode ? 'Award' : 'Deduct'} ${points} points?`,
        [
          { text: 'Cancel', style: 'cancel' },
          {
            ...
            onPress: () => {
              updatePoints({
                amount: points,
                operationType: isAddMode ? 'add' : 'subtract',
              });
              setCustomPoints('');
            },
      ...
      >
        ...
        <Text
          className={`text-lg font-medium ${
            item.operation_type === 'add' ? 'text-green-600' : 'text-red-600'
          }`}
        >
  ...
  const RewardCard = useCallback(({ reward }: { reward: RedeemedReward }) => {
    return (
      <Link
        href={`/admin/scannedProfile/reward/${reward.id}`}
            ...
            <View className='flex-1'>
              ...
              <View className='mt-1 flex-row items-center'>
                ...
                <Text className='ml-1 text-sm text-gray-600'>
                  {reward.config.points_needed_value} points
                </Text>
              </View>
            </View>
          </View>
          ...
          <ScrollView className='flex-1'>
            ...
            <View className='mb-4 flex-row space-x-4 px-4'>
              ...
              <StatCard
                title='Rewards Used'
                value={
                  redeemedRewards?.filter((r) => r.status === 'used').length ??
                  0
                }
                icon='gift'
                color='#2563EB'
              ...
              <View className='mb-4 flex-row justify-center space-x-2 rounded-xl bg-white p-2'>
                ...
                >
                  <AntDesign
                    name='plus'
                    size={18}
                    color={isAddMode ? 'white' : '#666'}
                  />
                  <Text
                    className={
                      isAddMode ? 'font-medium text-white' : 'text-gray-600'
                    }
                  >
                    Award
                  </Text>
              ...
              <View className='mb-4 rounded-xl bg-white p-4'>
                ...
                <View className='flex-row space-x-3'>
                  ...
                  <Pressable
                    onPress={() =>
                      customPoints &&
                      showConfirmationDialog(Number(customPoints))
                    }
              ...
              <View className='rounded-xl bg-white p-4'>
                ...
                <View className='-m-1 flex-row flex-wrap'>
                  {[10, 25, 50, 100, 250, 500].map((points) => (
                    <Pressable
                      key={points}
                      onPress={() => showConfirmationDialog(points)}
                      className='w-1/3 p-1'
                    >
                      <View
          ...
          <View className='flex-1 px-4'>
            ...
            <FlashList
              ...
              renderItem={({ item, index }) => (
                <TransactionItem
                  item={item}
                  isLastItem={index === (userHistory?.length ?? 0) - 1}
                />
              )}
          ...
          <View className='flex-1 px-4'>
            <View className='mb-4 flex-row space-x-4'>
              ...
              <StatCard
                ...
                value={redeemedRewards?.length ?? 0}
                icon='gift'
                color='#16A34A'
              />
            </View>
            ...
            >
              ...
              <Text
                className={`ml-2 font-medium ${
                  selectedSection === item.id ? 'text-white' : 'text-gray-600'
                }`}
              >
                {item.label}
              </Text>
