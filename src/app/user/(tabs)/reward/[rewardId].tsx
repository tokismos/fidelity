...
export default function RewardDetails() {
  ...
  const { userReward, isLoading, error } = useGetUserRewardById({
  ...
  const renderRewardValue = (reward: Reward) => {
    switch (reward.type) {
      case REWARD_TYPES.BUY_N_GET_1:
        return `Buy ${
          (reward.config as BuyNGet1Config).required_purchases
        } and get 1 free`;
        ...
        <View className='rounded-lg bg-white p-4 shadow'>
          <View className='mb-4 items-center'>
            {userReward.config.image_path ? (
              <Image
                source={{ uri: userReward.config.image_path }}
                className='h-24 w-24'
              />
            ) : (
              <View className='mb-2 rounded-full bg-blue-100 p-4'>
                <Ionicons
                  name={
                    userReward.type === 'DISCOUNT_PERCENTAGE' ||
                    userReward.type === 'DISCOUNT_FIX'
                      ? 'pricetag'
          ...
          <Text className='mb-4 text-center text-lg font-medium text-blue-600'>
            {renderRewardValue(userReward)}
          </Text>
