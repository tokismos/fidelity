...
const isBuyNGet1Config = (reward: UserReward): boolean =>
  reward.type === REWARD_TYPES.BUY_N_GET_1;

const isDiscountPercentageConfig = (reward: UserReward): boolean =>
  reward.type === REWARD_TYPES.DISCOUNT_PERCENTAGE;

const isDiscountFixConfig = (reward: UserReward): boolean =>
  reward.type === REWARD_TYPES.DISCOUNT_FIX;

const isFreeItemConfig = (reward: UserReward): boolean =>
  reward.type === REWARD_TYPES.FREE_ITEM;
...
const BuyNGet1ConfigComponent = ({ reward }: { reward: UserReward }) => {
  const { incrementPurchase } = useIncrementPurchases({
  ...
  return (
    <View className='space-y-2'>
      <Text className='text-sm text-gray-600'>
        Buy {(reward.config as BuyNGet1Config).required_purchases} items and get
        1 free
      </Text>

      <Text className='text-sm text-gray-600'>
        Points needed: {reward.config.points_needed_value}
      </Text>
      <Text className='text-sm text-gray-600'>
        YOU HAVE {reward.config.nbr_of_purchases}
      </Text>
      <Button title='Increment ' onPress={incrementPurchase} />
    </View>
  );
};
const DiscountPercentageConfigComponent = ({
  reward,
}: {
  reward: UserReward;
}) => (
  <View className='space-y-2'>
    <Text className='text-sm text-gray-600'>
      {(reward.config as DiscountPercentageConfig).discount_percentage}% off
      your purchase
    </Text>
    <Text className='text-sm text-gray-600'>
      Points needed: {reward.config.points_needed_value}
...
const FreeItemWithPurchaseConfigComponent = ({
  reward,
}: {
  reward: UserReward;
}) => (
  <View className='space-y-2'>
    <Text className='text-sm text-gray-600'>
      Free {(reward.config as FreeItemWithPurchaseConfig).free_item_name} with
      purchase of
